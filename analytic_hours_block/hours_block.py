...
class AccountHoursBlock(models.Model):
    ...
    def _get_last_action(self,  ids, name, arg, context=None):
        """ Return the last analytic line date for an invoice"""
        res = {}
        for block in self.browse( ids, context=context):
    ...
    def _compute_hours(self,  ids, fields, args, context=None):
        ...
        for block in self.browse( ids, context=context):
            ...
            for line in aal_obj.browse( line_ids, context=context):
                factor = 1.0
    ...
    def _compute_amount(self,  ids, fields, args, context=None):
        if context is None:
            context = {}
        result = {}
        aal_obj = self.pool.get('account.analytic.line')
    ...
    def _compute(self,  ids, fields, args, context=None):
        result = {}
        block_per_types = {}
        for block in self.browse( ids, context=context):
        ...
        for block_type in block_per_types:
            if block_type:
                ...
                result.update(func( ids, fields, args, context=context))
    ...
    def action_send_block(self,  ids, context=None):
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                 'analytic_hours_block', 'email_template_hours_block'
            )[1]
        except ValueError:
            template_id = False
    ...
    _recompute_triggers = {
        'account.hours.block': (lambda self,  ids, c=None:
    ...
    date_invoice= fields.Date(related=            'invoice_id.date_invoice',            type="date",            string="Invoice Date",            store={
                'account.hours.block': (lambda self,  ids, c=None: ids,
                                        ['invoice_id'], 10),
